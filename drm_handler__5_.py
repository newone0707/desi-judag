...
#........................................................................................................................................................................................
            ...
            if "acecwply" in url:
            ...
            elif any(x in url for x in ['classplusapp', 'testbook.com']):
                # ── ClassPlus / Testbook handler ──────────────────────────────
                base_url = url
                parsed   = urllib.parse.urlparse(url)
                qparams  = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)

                # Extract contentId — supports contentHashId, contentHashIdl, or path fallback
                contentId = (
                    qparams.get('contentHashId',  [None])[0] or
                    qparams.get('contentHashIdl', [None])[0] or
                    parsed.path.rstrip('/').split('/')[-1]
                )
                contentId = (contentId or '').strip()

                CP_API_URL = "https://api.classplusapp.com/cams/uploader/video/jw-signed-url"
                cp_headers = {
                    'host':              'api.classplusapp.com',
                    'x-access-token':    cptoken,
                    'accept-language':   'EN',
                    'api-version':       '23',
                    'app-version':       '1.5.18.1',
                    'build-number':      '50',
                    'content-type':      'application/json',
                    'device-details':    'Samsung_Galaxy S23_SDK-33',
                    'device-id':         'a1b2c3d4e5f60001',
                    'region':            'IN',
                ...
                is_drm = any(x in base_url for x in [
                    'classplusapp.com/drm', 'media-cdn.classplusapp.com/drm',
                    'testbook.com', '/drm/',
                ])

                try:
                    async with ClientSession() as sess:
                        async with sess.get(CP_API_URL, params=cp_params, headers=cp_headers, timeout=aiohttp.ClientTimeout(total=20)) as cp_resp:
                            cp_res = await cp_resp.json(content_type=None)

                    if is_drm:
                        drm_block = cp_res.get('drmUrls') or cp_res.get('drm') or {}
                        mpd_url   = drm_block.get('manifestUrl') or drm_block.get('mpd') or cp_res.get('url', base_url)
                        if mpd_url and mpd_url != base_url:
                            mpd, keys = helper.get_mps_and_keys(mpd_url)
                            url        = mpd
                            keys_string = " ".join(f"--key {k}" for k in keys)
                        else:
                            url         = cp_res.get('url', base_url)
                            keys_string = ""
                    else:
                        url         = cp_res.get('url', base_url)
                        keys_string = ""

                    print(f"ClassPlus API ✅ contentId={contentId[:30]}  drm={is_drm}")

                except Exception as e:
                    print(f"ClassPlus API ❌ {e}  url={base_url[:80]}")
                    url         = base_url
                    keys_string = ""
